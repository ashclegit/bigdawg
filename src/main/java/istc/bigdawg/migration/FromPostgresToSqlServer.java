 ...
 * @author akmr
 */
public class FromPostgresToSqlServer extends FromDatabaseToDatabase {

    /*
     * log
     */
    private static Logger logger = Logger
            .getLogger(FromPostgresToSqlServer.class);

    private String sqlServerPipe;

    /**
     * Always put extractor as the first task to be executed (while migrating
     * data from PostgreSQL to SQLServer).
     */
    ...
    public FromPostgresToSqlServer(ConnectionInfo connectionFrom,
     ...
     */
    public FromPostgresToSqlServer() {
        super();
    }
    ...
    public MigrationResult migrate(MigrationInfo migrationInfo)	throws MigrationException {
        ...
        if (migrationInfo.getConnectionFrom() instanceof PostgreSQLConnectionInfo
                && migrationInfo.getConnectionTo() instanceof SqlServerConnectionInfo) {
            try {
    ...
    private PostgreSQLSchemaTableName createTargetTableSchema(
        ...
        SQLServerHandler.executeStatement(connectionTo,
        ...
        if (createTableStatement == null) {
            ...
            createTableStatement = SQLServerPostgresTranslation.convertToSqlServer(postgresCreateTable);
            logger.debug("postgresCreateTable statement converted to SQLServer: " + createTableStatement);
        }
        SQLServerHandler.executeStatement(connectionTo, createTableStatement);
        return schemaTable;
    }
    ...
    public MigrationResult migrateSingleThreadCSV() throws MigrationException {
        ...
        try {
            ...
            conTo = SQLServerHandler.getConnection(getConnectionTo());
            ...
            sqlServerPipe = Pipe.INSTANCE.createAndGetFullName(
                    this.getClass().getName() + "_toSQLServer_" + getObjectFrom());

            List<Callable<Object>> tasks = new ArrayList<>();
            tasks.add(new ExportPostgres(migrationInfo.getConnectionFrom(), copyFromCommand,
            		sqlServerPipe, new SQLServerHandler(getConnectionTo())));
            tasks.add(new LoadSqlServer(conTo, migrationInfo, sqlServerPipe));
    ...
    public static void main (String[] args) {
        ...
        System.out.println("Migrating data from Postgres to SQLServer");
        ConnectionInfo conInfoTo = new SqlServerConnectionInfo("localhost",
                "3306", "test", "SQLServeruser", "test");
        ConnectionInfo conInfoFrom = new PostgreSQLConnectionInfo("localhost",
                "5432", "test", "pguser", "test");
        MigrationResult result;
        try {
        	FromPostgresToSqlServer migrator = new FromPostgresToSqlServer(conInfoFrom,
