...
import istc.bigdawg.migration.datatypes.SQLServerPostgresTranslation;
 ...
 * @author akmr
 */
public class FromSqlServerToPostgres extends FromDatabaseToDatabase {

    /*
     * log
     */
    private static Logger logger = Logger
            .getLogger(FromSqlServerToPostgres.class);

    private String sqlServerPipe;

    /**
     * Always put extractor as the first task to be executed (while migrating
     * data from SQLServer to PostgreSQL).
     */
    ...
    public FromSqlServerToPostgres(ConnectionInfo connectionFrom,
     ...
     */
    public FromSqlServerToPostgres() {
        super();
    }

    /**
     * Migrate data from SQLServer instance to Postgres.
     */
    public MigrationResult migrate(MigrationInfo migrationInfo)	throws MigrationException {
        ...
        if (migrationInfo.getConnectionFrom() instanceof SqlServerConnectionInfo
    ...
    private PostgreSQLSchemaTableName createTargetTableSchema(
        ...
        if (createTableStatement == null) {
            ...
            String sqlServerCreateTable = SQLServerHandler.getCreateTable(
                    connectionFrom, migrationInfo.getObjectFrom(),
                    migrationInfo.getObjectTo());
            logger.debug("sqlServerCreateTable statement: " + sqlServerCreateTable);
            createTableStatement = SQLServerPostgresTranslation.convertToPostgres(sqlServerCreateTable);
            logger.debug("sqlServerCreateTable statement converted to PostgreSQL: " + createTableStatement);
        }
    ...
    public MigrationResult executeMigration() throws MigrationException {
        ...
        try {
            conFrom = SQLServerHandler.getConnection(getConnectionFrom());
            ...
            sqlServerPipe = Pipe.INSTANCE.createAndGetFullName(
                    this.getClass().getName() + "_fromMySQL_" + getObjectFrom());

            List<Callable<Object>> tasks = new ArrayList<>();
            tasks.add(new ExportSqlServer(conFrom, sqlServerPipe, new PostgreSQLHandler(getConnectionTo()), migrationInfo));
            tasks.add(new LoadPostgres(conTo, migrationInfo, copyToCommand, sqlServerPipe));
    ...
    public static void main (String[] args) {
        ...
        System.out.println("Migrating data from SQLServer to PostgreSQL");
        ConnectionInfo conInfoFrom = new SqlServerConnectionInfo("localhost",
                "3306", "test", "mysqluser", "test");
        ConnectionInfo conInfoTo = new PostgreSQLConnectionInfo("localhost",
                "5432", "test", "pguser", "test");
        MigrationResult result;
        try {
        	FromSqlServerToPostgres migrator = new FromSqlServerToPostgres(conInfoFrom,
