 ...
 */
public class LoadSqlServer implements Load {

    /** For internal logging in the class. */
    private static Logger log = Logger.getLogger(LoadSqlServer.class);
    ...
    /**
     * Create task for loading data to SQLServer directly from a file (or named
     * pipe).
     *
     * @param connection
     ...
     *             encoding).
     */
    public LoadSqlServer(Connection connection, MigrationInfo migrationInfo, final String inputFile)
            throws SQLException {
    ...
    public Long call() throws Exception {
        ...
        try {
            ...
            String msg = e.getMessage()
                    + " Problem when loading data to SQLServer ";
    ...
    public void close() throws Exception {
        try {
            if (connection != null && !connection.isClosed()) {
                try {
                    ...
                    log.info("Could not commit any sql statement for "
                            + "the connection in LoadSqlServer. "
                            + e.getMessage());
                }
                connection.close();
                connection = null;
            ...
            String message = "Could not close the connection to SQLServer. "
                    + e.getMessage();
