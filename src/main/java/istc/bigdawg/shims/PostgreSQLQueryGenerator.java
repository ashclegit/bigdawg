...
public class PostgreSQLQueryGenerator implements OperatorQueryGenerator {
	...
	private void setAllowedScan(Set<String> allowedScans) {
		if (allowedScans != null)
			this.allowedScans = new HashSet<>(allowedScans);
	}
	...
	public void visit(Join joinOp) throws Exception {
		...
		if (join.generateJoinPredicate() != null) {
			jf.append(updateOnExpression(join.generateJoinPredicate(), (SQLIslandOperator) child0,
					(SQLIslandOperator) child1, t0, t1, true));
		} else {
			...
			if ((ses = processLeftAndRightWithIndexCond(join, true, discoveredJoinPredicate)) != null) {
				...
				if (stopAtJoin) {
					...
					String pred;
					while (!(leftChild instanceof Join) && leftChild.getChildren().size() > 0)
						leftChild = leftChild.getChildren().get(0);

					if (leftChild.getChildren().size() > 0
							&& (pred = ((Join) leftChild).generateJoinPredicate()) != null) {

						System.out.printf("::::::: Pred left: %s; \n", pred);

						Expression e = CCJSqlParserUtil.parseCondExpression(pred);
						List<String> es = SQLExpressionUtils.getAttributes(e).stream()
								.map(a -> a.getTable().getFullyQualifiedName()).collect(Collectors.toList());
						if ((new HashSet<>(child1ObjectMap.keySet())).removeAll(es)) {
							Expression exp = CCJSqlParserUtil.parseCondExpression(pred);
							SQLExpressionUtils.renameAttributes(exp, child1ObjectMap.keySet(), null,
									((Join) leftChild).getJoinToken());
							discoveredJoinPredicate.add(exp.toString());
						}
					}

					Operator rightChild = join.getChildren().get(1);
					while (!(rightChild instanceof Join) && rightChild.getChildren().size() > 0)
						rightChild = rightChild.getChildren().get(0);
					if (rightChild.getChildren().size() > 0
							&& (pred = ((Join) rightChild).generateJoinPredicate()) != null) {
						...
						if ((new HashSet<>(child0ObjectMap.keySet())).removeAll(es)) {
							Expression exp = CCJSqlParserUtil.parseCondExpression(pred);
							SQLExpressionUtils.renameAttributes(exp, child0ObjectMap.keySet(), null,
									((Join) rightChild).getJoinToken());
							discoveredJoinPredicate.add(exp.toString());
						}
					}
				}
				...
				if (join.generateJoinFilter() == null || join.generateJoinFilter().isEmpty()) {
					...
					try {
						List<Column> lc = SQLExpressionUtils
								.getAttributes(CCJSqlParserUtil.parseCondExpression(join.generateJoinFilter()));
						s = lc.get(0).getTable().getName();
						s2 = lc.get(1).getTable().getName();
					} catch (Exception e) {
						e.printStackTrace();
						throw new Exception(String.format(
								"Ses from Join gen dest only doesn't find match: %s; %s; %s;\n",
								((SQLIslandOperator) child0).getChildrenPredicates(),
								((SQLIslandOperator) child1).getChildrenPredicates(), join.generateJoinFilter()));
					}
				}
			}

			// System.out.printf("--->> join, discoveredJoinPredicate: %s;
			// joinPredicate: %s;\n"
			...
			if (child1.isPruned()) {
				...
				if (!s2.equals(temp[temp.length - 1]))
					t1.setAlias(new Alias(s2));
			}
		}
		...
		if (dstStatement == null && (child0.isPruned() || child0 instanceof Scan)) {
			...
			if (!(child1.isPruned() || child1 instanceof Scan))
				throw new Exception("child0 class: " + child0.getClass().getSimpleName().toString() + "; child1 class: "
						+ child1.getClass().getSimpleName().toString());
			...
			for (String s : ((SQLIslandOperator) child0).getOutSchema().keySet()) {
			...
			while (!treeWalker.isEmpty()) {
				...
				for (Operator o : treeWalker) {
					if (o instanceof Scan) {
						Expression f = ((SQLIslandScan) o).getFilterExpression();
						if (f != null && ((SQLIslandScan) o).isHasFunctionInFilterExpression()) {
							...
							SQLExpressionUtils.renameAttributes(e, aliases, aliases,
									((Aggregate) child1).getAggregateToken());
		...
		if (!discoveredJoinPredicate.isEmpty())
			for (String s : discoveredJoinPredicate)
				addToJoinFilter(s, jf); // there should be only one entry
		...
		if (jf.length() > 0 && !((estring = SQLExpressionUtils.getRelevantFilterSections(
				CCJSqlParserUtil.parseCondExpression(jf.toString()), child0.getDataObjectAliasesOrNames(),
			...
			for (Column c : filterRelatedTablesExpr) {
				...
				if (c.getTable().getAlias() != null)
					filterRelatedTables.add(c.getTable().getAlias().getName());
			}
			...
			while (!treeWalker.isEmpty()) {
				...
				for (Operator o : treeWalker) {
					if (o.isPruned()) {
						Set<String> aliasesAndNames = new HashSet<>(((SQLIslandOperator) o).getObjectAliases());
	...
	// }
	// saveRoot(join);
	// SQLIslandOperator child0 = (SQLIslandOperator)join.getChildren().get(0);
	// SQLIslandOperator child1 = (SQLIslandOperator)join.getChildren().get(1);
	//
	// // constructing the ON expression
	//
	// Table t0 = new Table();
	...
	//
	// if (join.generateJoinPredicate() != null) {
	// jf.append(updateOnExpression(join.generateJoinPredicate(), child0,
	// child1, t0, t1, true));
	// } else {
	// Map<String, String> child0ObjectMap =
	// child0.getDataObjectAliasesOrNames();
	// Map<String, String> child1ObjectMap =
	// child1.getDataObjectAliasesOrNames();
	//
	// String s, s2;
	// List<String> ses;
	// if ((ses = processLeftAndRightWithIndexCond(join, true,
	// discoveredJoinPredicate)) != null) {
	// s = ses.get(0);
	// s2 = ses.get(1);
	//
	// } else if ((ses = processLeftAndRightWithIndexCond(join, false,
	// discoveredJoinPredicate)) != null) {
	// s = ses.get(1);
	// s2 = ses.get(0);
	// } else {
	...
	// Operator leftChild = join.getChildren().get(0);
	// String pred;
	// while (!(leftChild instanceof Join) && leftChild.getChildren().size() >
	// 0) leftChild = leftChild.getChildren().get(0);
	//
	//
	// if (leftChild.getChildren().size() > 0 && (pred =
	// ((Join)leftChild).generateJoinPredicate()) != null) {
	//
	// System.out.printf("::::::: Pred left: %s; \n", pred);
	//
	// Expression e = CCJSqlParserUtil.parseCondExpression(pred);
	// List<String> es = SQLExpressionUtils.getAttributes(e).stream().map(a ->
	// a.getTable().getFullyQualifiedName()).collect(Collectors.toList());
	// if ((new HashSet<>( child1ObjectMap.keySet())).removeAll(es)) {
	// Expression exp = CCJSqlParserUtil.parseCondExpression(pred);
	// SQLExpressionUtils.renameAttributes(exp, child1ObjectMap.keySet(), null,
	// ((Join)leftChild).getJoinToken());
	...
	//
	// Operator rightChild = join.getChildren().get(1);
	// while (!(rightChild instanceof Join) && rightChild.getChildren().size() >
	// 0) rightChild = rightChild.getChildren().get(0);
	// if ( rightChild.getChildren().size() > 0 && (pred =
	// ((Join)rightChild).generateJoinPredicate()) != null) {
	// System.out.printf("::::::: Pred right: %s; \n", pred);
	// Expression e = CCJSqlParserUtil.parseCondExpression(pred);
	// List<String> es = SQLExpressionUtils.getAttributes(e).stream().map(a ->
	// a.getTable().getFullyQualifiedName()).collect(Collectors.toList());
	...
	//
	//
	// if (join.generateJoinFilter() == null ||
	// join.generateJoinFilter().isEmpty()) {
	// s = child0ObjectMap.keySet().iterator().next();
	// s2 = child1ObjectMap.keySet().iterator().next();
	// } else {
	// try {
	// List<Column> lc =
	// SQLExpressionUtils.getAttributes(CCJSqlParserUtil.parseCondExpression(join.generateJoinFilter()));
	// s = lc.get(0).getTable().getName();
	// s2 = lc.get(1).getTable().getName();
	// } catch (Exception e) {
	// e.printStackTrace();
	// throw new Exception(String.format("Ses from Join gen dest only doesn't
	// find match: %s; %s; %s;\n"
	// ,child0.getChildrenPredicates(), child1.getChildrenPredicates(),
	// join.generateJoinFilter()));
	// }
	// }
	// }
	//
	...
	//
	// boolean anyPruned = ((SQLIslandJoin)join).isAnyProgenyPruned();
	//
	// if (child0.isPruned()) {
	// t0.setName(child0.getPruneToken());
	// } else if (child0 instanceof Aggregate && stopAtJoin) {
	...
	// }
	//
	// if (dstStatement == null && (child0.isPruned() || child0 instanceof
	// Scan)) {
	//
	// // ensuring this is a left deep tree
	// if (!(child1.isPruned() || child1 instanceof Scan))
	// throw new Exception("child0 class:
	// "+child0.getClass().getSimpleName().toString()+"; child1 class:
	// "+child1.getClass().getSimpleName().toString());
	//
	// child0.accept(this);
	// } else if (child0 instanceof Aggregate && stopAtJoin) {
	// dstStatement = SelectUtils.buildSelectFromTable(new
	// Table(((Aggregate)child0).getAggregateToken()));
	// List<SelectItem> sil = new ArrayList<>();
	// for (String s : ((SQLIslandOperator)child0).getOutSchema().keySet()){
	// sil.add(new SelectExpressionItem(new Column(new
	// Table(((Aggregate)child0).getAggregateToken()), s)));
	// };
	// ((PlainSelect)dstStatement.getSelectBody()).setSelectItems(sil);
	// } else {
	// child0.accept(this);
	// }
	// // Resolve pruning and add join
	// if (child0.isPruned()) ((PlainSelect)
	// dstStatement.getSelectBody()).setFromItem(t0);
	// addJSQLParserJoin(dstStatement, t1);
	//
	//
	// if (child1 instanceof Aggregate && stopAtJoin) {
	...
	// populateComplexOutItem(join, false);
	// Expression e =
	// CCJSqlParserUtil.parseCondExpression(rewriteComplextOutItem(join, f));
	// Set<String> aliases = child1.getDataObjectAliasesOrNames().keySet();
	// SQLExpressionUtils.renameAttributes(e, aliases, aliases,
	// ((Aggregate)child1).getAggregateToken());
	// discoveredAggregateFilter = e.toString();
	...
	// Expression w = ((PlainSelect) dstStatement.getSelectBody()).getWhere();
	//
	// if (join.generateJoinFilter() != null || join.generateJoinPredicate() !=
	// null) {
	//
	// if (jf.length() == 0 && join.generateJoinPredicate() != null &&
	// join.generateJoinPredicate().length() > 0)
	// jf.append(updatePruneTokensForOnExpression(join,
	// join.generateJoinPredicate()));
	//
	// if (join.generateJoinFilter() != null &&
	// join.generateJoinFilter().length() > 0) {
	...
	// addToJoinFilter(discoveredAggregateFilter, jf);
	// if (!discoveredJoinPredicate.isEmpty()) for (String s :
	// discoveredJoinPredicate) addToJoinFilter(s, jf); // there should be only
	// one entry
	//
	//// System.out.printf("--->>>> join getRelevantFilterSections: \n\texpr:
	// %s;\n\tjf: %s;\n\tleft: %s,\n\tright: %s\n",
	...
	//
	//// System.out.printf("----> jf: %s\njoinPred: %s\njoinFilter: %s\n\n", jf,
	// join.generateJoinPredicate(), join.generateJoinFilter());
	//
	// if (jf.length() > 0 && !((estring =
	// SQLExpressionUtils.getRelevantFilterSections(
	...
	// Expression e = CCJSqlParserUtil.parseCondExpression(estring);
	//
	// List<Column> filterRelatedTablesExpr =
	// SQLExpressionUtils.getAttributes(e);
	// List<String> filterRelatedTables = new ArrayList<>();
	// for (Column c : filterRelatedTablesExpr) {
	// filterRelatedTables.add(c.getTable().getFullyQualifiedName());
	// if (c.getTable().getAlias() != null)
	...
	// List<Operator> nextGen;
	//
	// ((SQLIslandJoin)join).updateObjectAliases();
	//
	// while (!treeWalker.isEmpty()) {
	// nextGen = new ArrayList<>();
	// for (Operator o : treeWalker) {
	// if (o.isPruned()) {
	// Set<String> aliasesAndNames = new
	// HashSet<>(((SQLIslandOperator)o).getObjectAliases());
	// aliasesAndNames.addAll(o.getDataObjectAliasesOrNames().keySet());
	// Set<String> duplicate = new HashSet<>(aliasesAndNames);
	// if (aliasesAndNames.removeAll(filterRelatedTables))
	// SQLExpressionUtils.renameAttributes(e, duplicate, null,
	// o.getPruneToken());
	// } else
	// nextGen.addAll(o.getChildren());
	// }
	...
	//
	// ((PlainSelect)
	// dstStatement.getSelectBody()).setWhere(CCJSqlParserUtil.parseCondExpression(e.toString()));
	//
	// }
	//
	// }
	...
	public void visit(Sort sort) throws Exception {
		...
		SQLIslandSort sortOp = (SQLIslandSort) sort;

		saveRoot(sortOp);
		sortOp.getChildren().get(0).accept(this);

		if (sortOp.getChildren().get(0) instanceof SQLIslandJoin) {
			PlainSelect ps = (PlainSelect) dstStatement.getSelectBody();
	...
	public void visit(Distinct distinct) throws Exception {
		...
		SQLIslandDistinct distinctOp = (SQLIslandDistinct) distinct;
		...
		if (distinctOp.getChildren().get(0) instanceof Join) {
			...
			for (String alias : distinctOp.getOutSchema().keySet()) {
				Expression e = CCJSqlParserUtil
						.parseExpression(distinctOp.getOutSchema().get(alias).getExpressionString());
				SelectItem s = new SelectExpressionItem(e);
				if (!(e instanceof Column)) {
					((SelectExpressionItem) s).setAlias(new Alias(alias));
				}
	...
	public void visit(Scan scan) throws Exception {
		...
		SQLIslandScan scanOp = (SQLIslandScan) scan;
		...
		if (dstStatement == null) {
			if (!scanOp.isPruned() || rootStatus)
				...
				dstStatement = generateSelectWithToken(scanOp, scanOp.getPruneToken());// SelectUtils.buildSelectFromTable(new
																						// Table(scanOp.getPruneToken()));
		}
		...
		if (((PlainSelect) dstStatement.getSelectBody()).getSelectItems().size() == 1
			...
			for (String alias : scanOp.getOutSchema().keySet()) {
				...
				Expression e = CCJSqlParserUtil.parseExpression(
						rewriteComplextOutItem(scanOp, scanOp.getOutSchema().get(alias).getSQLExpression()));
				SelectItem s = new SelectExpressionItem(e);
		...
		if (scanOp.getFilterExpression() != null && (!scanOp.isPruned() || rootStatus)
				&& !scanOp.isHasFunctionInFilterExpression()) { // FilterExpression
																// with function
																// is pulled
	...
	public void visit(CommonTableExpressionScan cte) throws Exception {
		...
		// isSubTreeRoot, stopAtJoin, allowedScans);
		visit((Scan) cte);
		...
		if (!found) {
			...
			dstWiths.add(((SQLIslandCommonTableExpressionScan) cte).getWith());
			...
			cte.getSourceStatement().accept(this);// .generateSQLStringDestOnly(null,
													// true, stopAtJoin, new
													// HashSet<>());
	...
	public void visit(Aggregate aggregate) throws Exception {
		...
		SQLIslandAggregate aggregateOp = (SQLIslandAggregate) aggregate;

		saveRoot(aggregateOp);

		Select originalDST = null;
		if (dstStatement != null)
			originalDST = (Select) CCJSqlParserUtil.parse(dstStatement.toString());

		if (isRoot || aggregateOp.getAggregateID() == null)
			aggregateOp.getChildren().get(0).accept(this);// .generateSQLStringDestOnly(dstStatement,
															// false,
															// stopAtJoin,
															// allowedScans);
		else {
		...
		for (String alias : aggregateOp.getOutSchema().keySet()) {
			...
			Expression e = CCJSqlParserUtil.parseExpression(
					rewriteComplextOutItem(aggregateOp, aggregateOp.getOutSchema().get(alias).getSQLExpression()));
			SelectItem s = new SelectExpressionItem(e);
		...
		if (originalDST == null) {
			...
			originalDST = SelectUtils.buildSelectFromTable(new Table()); // immediately
																			// replaced
	...
	public void visit(Limit limit) throws Exception {
		...
		SQLIslandLimit limitOp = (SQLIslandLimit) limit;
		...
		if (limitOp.isLimitAll())
			sqllim.setLimitAll(limitOp.isLimitAll());
		else if (limitOp.isLimitNull())
			sqllim.setLimitNull(limitOp.isLimitNull());
		else
			sqllim.setRowCount(limitOp.getLimitCount());

		if (limitOp.isHasOffSet())
			sqllim.setOffset(limitOp.getLimitOffset());
	...
	public Pair<Operator, String> generateStatementForPresentNonMigratingSegment(Operator operator, boolean isSelect) throws Exception {
		...
		if (!(operator instanceof Join || operator instanceof Merge)
				&& (child instanceof Join || child instanceof Merge)) {
			// TODO targeted strike? CURRENTLY WASH EVERYTHING // Set<String>
			// names = child.getDataObjectNames();
			configure(true, true);
			...
			for (String s : ane.values())
				childAliasesAndNames.add(s);

			populateComplexOutItem((SQLIslandOperator) operator, false);
			...
			for (int i = 0; i < sil.size(); i++) {
				if (sil.get(i) instanceof SelectExpressionItem) {
					...
					sei.setExpression(CCJSqlParserUtil.parseExpression(
							rewriteComplextOutItem((SQLIslandOperator) operator, sei.getExpression())));
				}
			}
			...
			if (!isSelect)
				addSelectIntoToken(outputSelect, operator.getSubTreeToken());

			if (operator instanceof SQLIslandOperator && dstStatement != null) {
				postprocessSQLStatement((SQLIslandOperator) operator);
			}
	...
	public String generateSelectIntoStatementForExecutionTree(String into) throws Exception {
		if (root == null)
			throw new Exception("SQLQueryGenerator, selectinto: root is null");
		postprocessSQLStatement((SQLIslandOperator) root);
	...
	protected void postprocessSQLStatement(SQLIslandOperator operator) throws Exception {
		...
		// operator.accept(this);
		// Select dstStatement = this.generateSQLStringDestOnly(null, true,
		// stopAtJoin, operator.getDataObjectAliasesOrNames().keySet());
		...
		// changeAttributesForSelectItems(operator,
		// srcStatement.getSelectBody(), dstStatement.getSelectBody(), selects,
		// stopAtJoin);
		// else {
		...
		for (String s : ((SQLIslandOperator) operator).getOutSchema().keySet()) {
			SelectExpressionItem sei = new SelectExpressionItem();
			sei.setExpression(CCJSqlParserUtil
					.parseExpression(((SQLIslandOperator) operator).getOutSchema().get(s).getExpressionString()));
			lsi.add(sei);
		}
			...
			for (SelectBody sb : psl) {
				...
				while (!walker.isEmpty()) {
					...
					for (Operator o : walker) {
						...
						if (!o.isPruned() && !(stopAtJoin && o instanceof Join) && !(stopAtJoin
								&& o instanceof Aggregate && ((Aggregate) o).getAggregateID() != null)) {
							nextgen.addAll(o.getChildren());
							continue;
						}
						...
						for (int i = 0; i < sil.size(); i++) {
							if (sil.get(i) instanceof SelectExpressionItem) {
								SelectExpressionItem sei = (SelectExpressionItem) sil.get(i);
								sei.setExpression(CCJSqlParserUtil
										.parseExpression(rewriteComplextOutItem(operator, sei.getExpression())));
							}
						}
						...
						if (dstStatement.getWithItemsList() != null)
							for (WithItem wi : dstStatement.getWithItemsList())
								updateSubTreeTokens(((PlainSelect) wi.getSelectBody()), childAliases,
										childAliasesAndNames, token);
	...
	private void changeAttributesForSelectItems(SQLIslandOperator o, SelectBody ss, SelectBody ps,
			Map<String, SelectItem> selects, boolean stopAtJoin) throws Exception {
		...
		if (ps instanceof SetOperationList) {
			...
			for (int i = 0; i < o.getChildren().size(); ++i) {
				...
				SQLIslandOperator c = (SQLIslandOperator) o.getChildren().get(i);
				PlainSelect p = ((PlainSelect) ((SetOperationList) ps).getSelects().get(i));

				changeAttributesForSelectItems(c, null, p, selects, stopAtJoin); // TODO
																					// SS?????
			}
	 ...
	 * @throws Exception
	 */
	private boolean changeAttributeName(SQLIslandOperator operator, SQLAttribute attr, boolean stopAtJoin)
			throws Exception {
		...
		for (Column c : attribsExpr)
			attribs.add(c.getFullyQualifiedName());
			...
			for (Operator op : operator.getChildren()) {
				...
				if (o.isPruned()) {
					...
					if (o.getOutSchema().containsKey(attr.getName()) || attribs.removeAll(o.getOutSchema().keySet())) {
						...
						if (e instanceof Column)
							((Column) e).setTable(new Table(((Join) o).getJoinToken()));
						else
							e = new Column(new Table(((Join) o).getJoinToken()), attr.getName());
					...
					// REDO: change the entire expression to match the output of
					// children aggregates
					...
					if (o.getOutSchema().containsKey(attr.getName()) || attribs.removeAll(o.getOutSchema().keySet())) {
						...
						if (e instanceof Column)
							((Column) e).setTable(new Table(((Aggregate) o).getAggregateToken()));
						else
							e = new Column(new Table(((Aggregate) o).getAggregateToken()), attr.getName());
					...
					if (changeAttributeName(o, attr, stopAtJoin))
						return true;
				}
			}
		}
		...
		if (into != null) {
			...
			for (WithItem wi : dstStatement.getWithItemsList()) {
				if (wi.getName().equals(into)) {
					...
					dstStatement.getWithItemsList().remove(wi); // remove this
																// item so it no
																// longer gets
																// bundled
	...
	private void addInto(SelectBody body, String into) {
		...
		SelectItemVisitor siv = new SelectItemVisitor() {
			...
			} // this is bad, but we can't do too much about it

			@Override
			public void visit(SelectExpressionItem selectExpressionItem) {
				try {
					List<String> columns = SQLExpressionUtils
							.getColumnNamesInAllForms(selectExpressionItem.getExpression());
					if (!columns.removeAll(columnNames)) {
						columnNames.addAll(columns); // EXTREMELY BAD PRACTICE.
														// BETTER SOLUTION
														// NEEDED
	...
	private List<SelectItem> changeSelectItemsOrder(SelectBody ss, Map<String, SelectItem> selects) throws Exception {
		...
		SelectItemVisitor siv = new SelectItemVisitor() {
			...
			public void visit(SelectExpressionItem selectExpressionItem) {
				...
				// find the child where the pruned token or seqscan or CTE is
				// located, make it the corresponding position
					...
					public void visit(Column tableColumn) {
						...
						if (selects.get(out) != null)
							holder.add(selects.get(out));
								...
								out = tableColumn.getTable().getName() + "." + tableColumn.getColumnName()) != null)
							holder.add(selects.get(out));
						else {
	 ...
	 * @throws Exception
	 */
	private void updateSubTreeTokens(PlainSelect ps, Set<String> originalAliases, Set<String> aliasesAndNames,
			String subTreeToken) throws Exception {
		...
		if (where != null)
			SQLExpressionUtils.renameAttributes(where, originalAliases, aliasesAndNames, subTreeToken);
		if (having != null)
			SQLExpressionUtils.renameAttributes(having, originalAliases, aliasesAndNames, subTreeToken);

		// CHANGE ORDER BY
		if (obes != null && !obes.isEmpty())
			for (OrderByElement obe : obes)
				SQLExpressionUtils.renameAttributes(obe.getExpression(), originalAliases, aliasesAndNames,
						subTreeToken);
		...
		for (SelectItem si : sis) {
			SelectItemVisitor siv = new SelectItemVisitor() {
				...
				public void visit(SelectExpressionItem selectExpressionItem) {
					try {
						SQLExpressionUtils.renameAttributes(selectExpressionItem.getExpression(), originalAliases,
								aliasesAndNames, subTreeToken);
					} catch (JSQLParserException e) {
						e.printStackTrace();
					}
				}
		...
		FromItemVisitor fv = new FromItemVisitor() {
			...
			public void visit(SubSelect subSelect) {
				try {
					updateSubTreeTokens((PlainSelect) subSelect.getSelectBody(), originalAliases, aliasesAndNames,
							subTreeToken);
				} catch (Exception e) {
					e.printStackTrace();
				}
	...
	private Select generateSelectWithToken(Operator o, String token) throws Exception {
		...
		if (token != null)
			outStatement = SelectUtils.buildSelectFromTable(new Table(token));
		else if (o instanceof SQLIslandScan)
			outStatement = SelectUtils.buildSelectFromTable(((SQLIslandScan) o).getTable());
		else
			throw new Exception("Unhandled token name in generateSelectWithToken; operator: "
					+ o.getClass().getSimpleName() + "; " + o.toString());

		PlainSelect ps = (PlainSelect) outStatement.getSelectBody();
		List<SelectItem> lsi = new ArrayList<>();

		for (String s : ((SQLIslandOperator) o).getOutSchema().keySet()) {
			SelectExpressionItem sei = new SelectExpressionItem();
			Expression e = CCJSqlParserUtil
					.parseExpression(((SQLIslandOperator) o).getOutSchema().get(s).getExpressionString());
			if (token != null)
	 ...
	 * @param first
	 */
	private void populateComplexOutItem(SQLIslandOperator o, boolean first) {
		// populate complexOutItemFromProgeny
		for (Operator c : o.getChildren()) {
			SQLIslandOperator child = (SQLIslandOperator) c;
			if ((!first) && child.getComplexOutItemFromProgeny().isEmpty())
				populateComplexOutItem(child, first);
			for (String s : child.getOutSchema().keySet()) {
				...
				if (e instanceof Column)
					continue;
				o.getComplexOutItemFromProgeny().put(s, e.toString().replaceAll("[.]", "\\[\\.\\]")
						.replaceAll("[(]", "\\[\\(\\]").replaceAll("[)]", "\\[\\)\\]"));
			}
			if (first || (!child.isPruned() || (child instanceof Join && ((Join) child).getJoinID() == null)
					|| (child instanceof Aggregate && ((Aggregate) child).getAggregateID() == null)))
				o.getComplexOutItemFromProgeny().putAll(child.getComplexOutItemFromProgeny());
		}
	}
	...
	// * @throws Exception
	// */
	// private String rewriteComplextOutItem(Operator o, String expr) throws
	// Exception {
	// // simplify
	// expr = CCJSqlParserUtil.parseExpression(expr).toString();
	// for (String alias : o.getComplexOutItemFromProgeny().keySet()) {
	// expr =
	// expr.replaceAll("("+o.getComplexOutItemFromProgeny().get(alias)+")",
	// alias);
	// }
	// return expr;
	// }
	 ...
	 * @throws Exception
	 */
	private String rewriteComplextOutItem(SQLIslandOperator o, Expression e) throws Exception {
		// simplify
	 ...
	 * @throws Exception
	 */
	private String updateOnExpression(String joinPred, SQLIslandOperator child0, SQLIslandOperator child1, Table t0,
			Table t1, boolean update) throws Exception {
	 ...
	 * @throws Exception
	 */
	private boolean replaceTableNameWithPruneName(SQLIslandOperator child, Expression e, Table t, List<String> itemsSet)
			throws Exception {
		if (child.isPruned()) {
			// does child have any of those names?
			Set<String> names = new HashSet<>(child.getDataObjectAliasesOrNames().keySet());
			if (child.getObjectAliases() == null)
			...
			for (Operator o : child.getChildren()) {
				ret = ret || replaceTableNameWithPruneName(((SQLIslandOperator) o), e, t, itemsSet);
			}
			return ret;
		}
	}
	 ...
	 * @throws Exception
	 */
	private boolean findAndGetTableName(SQLIslandOperator child, Table t, List<String> itemsSet) throws Exception {
		...
		if (names.size() > 0) {
			if (child instanceof Scan) {
				t.setName(((SQLIslandScan) child).getTable().toString());
			} else if (child.getChildren().size() > 0) {
				return findAndGetTableName((SQLIslandOperator) child.getChildren().get(0), t, itemsSet);
			}
			return false;
		} else {
			boolean ret = false;
			for (Operator o : child.getChildren()) {
				ret = ret || findAndGetTableName((SQLIslandOperator) o, t, itemsSet);
			}
			return ret;
		}
	}
	 ...
	 * @throws Exception
	 */
	private List<String> processLeftAndRightWithIndexCond(SQLIslandOperator o, boolean zeroFirst,
			List<String> foundExpression) throws Exception {

		Map<String, Expression> child0Cond;
		Map<String, Expression> child1Cond;

		if (zeroFirst) {
			child0Cond = ((SQLIslandOperator) o.getChildren().get(0)).getChildrenPredicates();
			child1Cond = ((SQLIslandOperator) o.getChildren().get(1)).getChildrenPredicates();
		} else {
			child0Cond = ((SQLIslandOperator) o.getChildren().get(1)).getChildrenPredicates();
			child1Cond = ((SQLIslandOperator) o.getChildren().get(0)).getChildrenPredicates();
		}

		for (String s : child0Cond.keySet()) {
			if (child0Cond.get(s) == null)
				continue;
			...
			for (Column c : ls) {
				...
				if (child1Cond.containsKey(s2)) {
					...
					// processLeftAndRightWithIndexCond, found expression:
					// %s\n\n", foundExpression);
					return ret;
				}
			}
	 ...
	 * @return
	 */
	private SubSelect makeNewSubSelectUpdateDST(SQLIslandAggregate agg, Select dstStatement) {
	...
	private String updatePruneTokensForOnExpression(SQLIslandJoin j, String joinPred) throws Exception {

		if (!j.isAnyProgenyPruned())
			return new String(joinPred);
	...
	public List<String> getJoinPredicateObjectsForBinaryExecutionNode(Join join) throws Exception {
		...
		if (join.generateJoinPredicate() == null && join.generateJoinFilter() == null) {
			...
			if (ses != null) {
				...
				s = ses.get(1);
				s2 = ses.get(0);
				extraction = ((SQLIslandOperator) join.getChildren().get(1)).getChildrenPredicates().get(s2);
			}
			...
			for (Expression ex : exprs) {
				...
				if (ex instanceof OrExpression)
					throw new Exception("SQLQueryGenerator: OrExpression encountered");
				if (!(ex instanceof EqualsTo))
					throw new Exception("SQLQueryGenerator: Non-EqualsTo expression encountered");

				List<Column> ls = SQLExpressionUtils.getAttributes(ex);
				for (Column c2 : ls)
					if (c2.getTable().getName().equals(s)) {
						leftColumn = c2;
						break;
					}
				for (Column c2 : ls)
					if (c2.getTable().getName().equals(s2)) {
						rightColumn = c2;
						break;
					}

				while (extraction instanceof Parenthesis)
					extraction = ((Parenthesis) extraction).getExpression();
				// ret.add(SQLExpressionUtils.getBinaryExpressionOperatorToken(ex));
				// ret.add(String.format("{%s, %s}",
				// leftColumn.getTable().getFullyQualifiedName(),leftColumn.getColumnName()));
				// ret.add(String.format("{%s, %s}",
				// rightColumn.getTable().getFullyQualifiedName(),rightColumn.getColumnName()));
		...
		// System.out.println("---> Left Child objects:
		// "+leftChildObjects.toString());
		// System.out.println("---> Right Child objects:
		// "+rightChildObjects.toString());
		// System.out.println("---> joinPredicate: "+joinPredicate);

		String preds = join.generateJoinPredicate();
		if (preds == null)
			preds = join.generateJoinFilter();
		if (preds == null)
			return ret;
		...
		for (Expression ex : exprs) {
			...
			if (leftChildObjects.contains(left.getTable().getName())
					|| leftChildObjects.contains(left.getTable().getFullyQualifiedName())) {
				// ret.add(String.format("{%s, %s}",
				// left.getTable().getFullyQualifiedName(),left.getColumnName()));
				// ret.add(String.format("{%s, %s}",
				// right.getTable().getFullyQualifiedName(),right.getColumnName()));
			...
			// System.out.printf("---> SQLQueryGenerator joinPredicate ret:
			// %s\n", ret.toString());
		}
	...
	// public class UnionOp extends SetOperation {boolean isUnionAll = false;
	// public UnionOp(SetOperationType type) {super(SetOperationType.UNION);}
	// public UnionOp(boolean isUnionAll)
	// {super(SetOperationType.UNION);this.isUnionAll = isUnionAll;}}
