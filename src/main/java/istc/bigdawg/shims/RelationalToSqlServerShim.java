 ...
 * @author akmr
 */
public class RelationalToSqlServerShim implements Shim {
    private SqlServerConnectionInfo ci;
    private SQLServerHandler handler = null;

    @Override
    public void connect(ConnectionInfo ci) throws ShimException {
        if (ci instanceof SqlServerConnectionInfo) {
            this.ci = (SqlServerConnectionInfo) ci;
            handler = new SQLServerHandler(this.ci);
        } else {
            throw new ShimException("Input Connection Info is not SqlServerConnectionInfo; rather: "+ ci.getClass().getName());
        }
    }
